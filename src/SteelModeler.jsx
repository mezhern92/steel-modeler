...
function parseText(t){
  const s=t.toLowerCase(); const num=(re,d)=>{const m=s.match(re);return m?parseFloat(m[1]):d;};
  const eave=num(/(\d+(?:\.\d+)?)\s*m(?:eters?)?\s*(?:height\s*)?(?:in\s*)?at\s*(?:the\s*)?edges?/,6)||num(/eave\s*(?:height\s*)?(?:of\s*)?(\d+(?:\.\d+)?)/,6);
  const ridge=num(/(\d+(?:\.\d+)?)\s*m?\s*at\s*(?:the\s*)?cent(?:re|er)/,8)||num(/ridge\s*(?:height\s*)?(?:of\s*)?(\d+(?:\.\d+)?)/,8);
  const baysShort=num(/(\d+)\s*bays?\s*in\s*(?:the\s*)?short/,2);
  const baysLong=num(/(\d+)\s*bays?\s*in\s*(?:the\s*)?long/,8);
  const spacing=num(/spacing\s*(?:of\s*)?(\d+(?:\.\d+)?)/,6);
  let Fy=250, grade="A36";
  if(/a992|gr\.?\s*50|s355/.test(s)){Fy=345;grade="A992/S355";}
  if(/a36/.test(s)){Fy=250;grade="A36";}
  return {eave,ridge,baysShort,baysLong,spacing,Fy,grade,V:42,cladding:0.15,services:0.10,Lr:0.96,driftLim:100,deflLim:240,purlinSp:1.0};
}
...
function engine(P){
  ...
  const qh=0.613*0.929*0.85*P.V*P.V/1000, G=0.85, deg=slope*180/Math.PI;
  const mkW=(pi)=>{
...
function s2kR3(P,R){
  ...
  for(let f=0;f<nF;f++)colX.forEach(x=>p(`   Joint=${J[`b${f}_${x}`]}   U1=Yes   U2=Yes   U3=Yes   R1=No   R2=No   R3=No`));p('');
...
export default function App(){
  ...
  const tabs=[["model","Drawing"],["loads","Loads & combos"],["design","Design"],["export","Export"]];
